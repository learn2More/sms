 ...
 **/
@Api(value = "xwProviderService", description = "the xwProviderService")
@Controller
@RequestMapping(value = "/xwProviderService")
public class XWProviderServiceImpl implements SmsProviderService {

    private final static Logger logger = LoggerFactory.getLogger(XWProviderServiceImpl.class);

    @Autowired
    XWProviderBiz xwProviderBiz;

    @ApiOperation(value = "玄武短信发送", notes = "xwProviderService.send", response = SmsResponse.class, tags = {"xwProviderService.send"})
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "玄武短信发送", response = SmsResponse.class)})
    ...
    public @ResponseBody SmsResponse send(@ApiParam(value = "request", required = true) @RequestBody SmsRequest request) {
        ...
        try {
            ...
            Cat.logMetricForDuration("服务商接口调用耗时,XuanwuSmsSend",
